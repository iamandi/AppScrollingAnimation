...
import { TAB_HEIGHT, CONVER_EXTRA_HEIGHT_FOR_IPHONEX } from "../../constants";

import { LIST_ITEM_KIND, type ListScrollPositionsType } from "../../types";
...
const ListRenderer = ({
  ...
  const { items, itemCount, cover, scrollAnimation } = useContext(ListContext);

  const keyExtractor = useCallback(
    (item, index) => `list-${item.kind}-${index}`,
    []
  );
  const listHeaderComponent = useMemo(
    () => <Cover data={cover} scrollAnimation={scrollAnimation} />,
  ...
  const onLayoutContent = useCallback(
    (index: number, height: number) => {
      ...
      const visibleContentHeights = contentHeights.filter(
      ...
      if (visibleContentHeights.length === itemCount) {
        ...
        visibleContentHeights.forEach((contentHeight, contentIndex) => {
          ...
          if (contentIndex === 0)
            start += TAB_HEIGHT - CONVER_EXTRA_HEIGHT_FOR_IPHONEX - 10;
  ...
  const renderItem = useCallback(
    ({ item, index }) => {
      const { kind, title, contents } = item;
