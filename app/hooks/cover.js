...
export function useCoverSize(): CoverSizeType {
  ...
  const extra = useMemo(() => {
    const coverHeight = Math.floor(
      (COVER_BASE_HEIGHT * coverWidth) / COVER_BASE_WIDTH
    );
    const minimumCoverHeight =
      Math.floor(coverHeight * 0.6) + CONVER_EXTRA_HEIGHT_FOR_IPHONEX;
...
export function useCoverStyles(
  ...
  const { coverWidth, coverHeight, minimumCoverHeight } = useCoverSize();
        ...
        transform: [
          {
            translateY: scrollAnimation.interpolate({
              ...
              outputRange: [
                0,
                COVER_TITLE_IMAGE_HEIGHT - minimumCoverHeight / 4,
              ],
              extrapolate: "clamp",
            }),
          },
