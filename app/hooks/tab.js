...
export function useTabStyles(
  ...
  const { container: baseContainerStyle, scroll: baseScrollStyle } = baseStyles;

  const { minimumCoverHeight } = useCoverSize();
  const { scrollAnimation, tabItemWidth, tabItemTotalWidth } = useContext(
    ListContext
  );
...
export function useTabItems() {
  ...
  const tabItems = useMemo(
    () =>
      items.map<any>(({ tabTitle }, index) => (
        <TabItem key={`item-${index}`} index={index} title={tabTitle} />
      )),
    [items]
  );
